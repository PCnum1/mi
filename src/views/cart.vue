<template>
    <div class="cart">
        <order-header title="我的购物车">
            <template v-slot:tip>
                <span>温馨提示：产品是否否买成功，以最终下单为准，请尽快结算</span>
            </template>
        </order-header>
        <div class="wrapper">
            <div class="container">
        ...
        methods:{
            ...
            updateCart(item,type){
                let quantity = item.quantity,
                  ...
                  if(type == '-'){
                    if(quantity == 1){
                        this.$message({message:'商品至少保留一件',type:'warning',duration:'800'})
                        return;
                    }
                    --quantity;
                  }else if(type == '+'){
                  if(quantity > item.productStock){
                    this.$message({message:'购买数量不能超过库存数量',type:'warning',duration:'800'})
                      return;
                  }
                  ++quantity;
                  }else{
