        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div @click="checkIndex = index" :class="{'checked':index === checkIndex}" class="addr-info" v-for="(item,index) in list" :key="index">
                <h2>{{item.receiverName}}</h2>
                <div class="phone">{{item.receiverMobile}}</div>
                <div class="street">{{item.receiverProvince + ' ' + item.receiverCity + ' ' + item.receiverDistrict + ' ' + item.receiverAddress}}</div>
                <div class="action">
                  <a href="javascript:;" class="fl">
                    <svg class="icon icon-del"><use xlink:href="#icon-del" @click="delAddress(item)"></use></svg>
                  </a>
                  <a href="javascript:;" class="fr">
                    <svg class="icon icon-edit"><use xlink:href="#icon-edit" @click="editAddressModal(item)"></use></svg>
                  </a>
                </div>
              </div>
